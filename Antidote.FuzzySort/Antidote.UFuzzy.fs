...
let uFuzzy: FuzzySortStatic = jsNative

[<AllowNullLiteral>]
type IExports =
    abstract FuzzySort: FuzzySortStatic

[<AllowNullLiteral>]
type FuzzySort =
    /// initial haystack filter, can accept idxs from previous prefix/typeahead match as optimization
    abstract filter: haystack: Array<string> * needle: string * ?idxs: FuzzySort.HaystackIdxs -> FuzzySort.HaystackIdxs
    ...
    abstract info: idxs: FuzzySort.HaystackIdxs * haystack: ResizeArray<string> * needle: string -> FuzzySort.Info
    /// performs final result sorting via Array.sort(), relying on Info
    abstract sort: info: FuzzySort.Info * haystack: ResizeArray<string> * needle: string -> FuzzySort.InfoIdxOrder
    /// utility for splitting needle into terms following defined interSplit/intraSplit opts. useful for out-of-order permutes
    abstract split: needle: string -> FuzzySort.Terms

[<AllowNullLiteral>]
type FuzzySortStatic =
    [<EmitConstructor>]
    abstract Create: FuzzySort.IFuzzySortOptions -> FuzzySort
...
module FuzzySort =
    ...
    [<AllowNullLiteral>]
    [<Global>]
    type IFuzzySortOptions [<ParamObject; Emit("$0")>]
        (
            ?interLft: BoundMode,
            ?interRgt: BoundMode,
